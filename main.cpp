...
void testHilbertMapColours()
{
	float lengthScale = 4000.0f;
	float learningRate = 0.000001f;
	float regularisationLambda = 0;

	OccupancyFileReader fileReader("/home/henry/ICPCUDA_results/run4/config.ini");
	
	fileReader.parse();
	Eigen::MatrixXf points = fileReader.getPoints();
	Eigen::MatrixXi occupancy = fileReader.getOccupancy();

	std::vector<int> r = fileReader.getR();
	std::vector<int> g = fileReader.getG();
	std::vector<int> b = fileReader.getB();
	
	std::vector<float> xSurf = fileReader.getXSurf();
	std::vector<float> ySurf = fileReader.getYSurf();
	std::vector<float> zSurf = fileReader.getZSurf();
	
	HilbertMap hm(lengthScale, points, occupancy, r, g, b, xSurf, ySurf, zSurf);
	hm.train(learningRate, regularisationLambda);
